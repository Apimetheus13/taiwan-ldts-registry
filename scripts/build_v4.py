...
# audit
tot = len(rows)
manual_n = sum(1 for r in rows if str(r[19]).startswith("人工核對補齊"))
...
d4 = [
    ...
    ["核對完成度", f"32 家／{tot} 個認證項目", "全數完成",
     f"全部 32 家皆由認證附件 PDF 原檔解析，無人工轉錄項目。其中 {tot - manual_n} 列由自動解析並經雙引擎交叉驗證，"
     f"{manual_n} 列因附表逐字錯位改以原檔人工核對補齊。"],
    ["交叉驗證", f"檢測名稱 {tot} 列 × pypdf 獨立引擎", "全數通過",
     "以另一套 PDF 引擎重新抽取原文比對，所有檢測名稱皆可於原檔找到。"],
    ["交叉驗證", f"儀器序號約 {nser} 組 × pypdf 獨立引擎", f"{okn}/{tot} 列完成",
     f"其中 {tot - manual_n} 列自動驗證通過；{manual_n} 列因附表逐字排版導致鄰欄文字穿插或無法擷取，"
     "已開原檔以第二套引擎逐字對照復原，標示為「人工核對補齊」，未以推測填補。"],
